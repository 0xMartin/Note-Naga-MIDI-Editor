...
MidiKeyboardRuler::MidiKeyboardRuler(int min_note_, int max_note_, int viewer_row_height_, QWidget* parent)
    : QWidget(parent),
      min_note(min_note_),
      max_note(max_note_),
...
std::vector<int> MidiKeyboardRuler::white_keys() const {
    ...
    for (int n = max_note; n >= min_note; --n)
        if (is_white_key(n)) result.push_back(n);
    return result;
}
std::vector<int> MidiKeyboardRuler::black_keys() const {
    std::vector<int> result;
    for (int n = max_note; n >= min_note; --n)
        if (is_black_key(n)) result.push_back(n);
    return result;
}
...
void MidiKeyboardRuler::highlight_keys_slot(QMap<int, QColor> note_color_dict) {
    highlight_keys(note_color_dict);
}

void MidiKeyboardRuler::highlight_keys(QMap<int, QColor> note_color_dict) {
    for (auto it = note_color_dict.begin(); it != note_color_dict.end(); ++it) {
        int note = it.key();
        QColor color = it.value();
        key_highlights[note] = color;
        // Správa timerů: pokud bys chtěl timeout, zde přidej logiku.
        // (timeout zde není, jen barva)
